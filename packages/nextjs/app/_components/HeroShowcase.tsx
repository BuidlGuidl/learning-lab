...
// Question, Experiment, Summary — and the Socratic AI tutor, each with a tiny
...
const ExerciseEx = () => (
  <div className="flex h-full flex-col gap-2.5">
    ...
    <div className="mt-auto flex items-center gap-2.5">
      <span className="inline-flex items-center gap-1.5 rounded-buttons bg-lp-btn-bg px-3 py-1.5 text-[12px] font-bold text-lp-on-accent">
        <PlayIcon className="h-3 w-3" />
        Run
      </span>
      <span className="inline-flex items-center gap-1 text-[11.5px] font-bold text-lp-trust-check">
...
const QUESTION_ANSWER = "In the contract's balance?";
...
const QuestionEx = ({ active }: { active: boolean }) => {
  ...
  return (
    <div className="flex h-full flex-col justify-center gap-2">
      <div className="lp-bubble max-w-[94%] rounded-[13px] rounded-bl-[6px] bg-lp-bubble-tutor px-3.5 py-2 text-[12.5px] leading-[1.5] text-lp-text-primary">
        After someone calls <code>fund()</code>, where does their ETH actually sit?
      </div>
      ...
      <span
        className={cn(
          ...
          phase === "checking"
            ? "bg-lp-btn-bg text-lp-on-accent"
            : "border border-lp-ghost-border text-lp-text-primary",
        )}
      >
      ...
      <div
        className={cn(
          "lp-bubble flex max-w-[94%] items-start gap-1.5 rounded-[13px] rounded-bl-[6px] bg-lp-bubble-tutor px-3.5 py-2 text-[12.5px] leading-[1.5] text-lp-text-primary transition-all duration-300",
      ...
      >
        ...
        <span>{"Right — it sits in the contract's own balance until the code sends it out."}</span>
      </div>
    </div>
  );
};
...
const ExperimentEx = () => (
  <div className="flex h-full flex-col gap-2.5">
    <span className="inline-flex items-center self-start gap-1.5 rounded-buttons bg-lp-btn-bg px-3.5 py-2 text-[12.5px] font-bold text-lp-on-accent">
...
const SUMMARY_ITEMS = [
  "A network nobody owns",
  "Accounts, gas, and onchain state",
  "A real contract you wrote and shipped",
];

const SummaryEx = () => (
  <ul className="m-0 flex h-full list-none flex-col justify-center gap-2.5 p-0">
    {SUMMARY_ITEMS.map(item => (
      <li key={item} className="flex items-center gap-2 text-[13px] text-lp-text-primary">
        <CheckCircleIcon className="h-4 w-4 shrink-0 text-lp-trust-check" />
        {item}
      </li>
    ))}
  </ul>
...
const TutorEx = () => (
  <div className="flex h-full flex-col justify-center gap-2">
    <div className="lp-bubble max-w-[94%] rounded-[13px] rounded-bl-[6px] bg-lp-bubble-tutor px-3.5 py-2.5 text-[12.5px] leading-[1.5] text-lp-text-primary">
      Your <code>refund()</code> reverts. What should <code>contributions[msg.sender]</code> be at this point?
    </div>
    <div className="max-w-[94%] self-end rounded-[13px] rounded-br-[6px] border border-lp-bubble-you-border bg-lp-bubble-you px-3.5 py-2.5 text-[12.5px] leading-[1.5] text-lp-text-primary">
...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    body: "One click ships your contract to an in-browser chain. Then call it from real accounts and watch the state move.",
  ...
  },
  {
    ...
    body: "Every chapter ends by connecting what you built to why it matters.",
    example: SummaryEx,
  },
];
...
export const HeroShowcase = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 border-t border-lp-shot-border bg-lp-shot-bar px-3 py-2.5">
        {SLIDES.map((slide, i) => (
          <button
            key={slide.key}
            type="button"
