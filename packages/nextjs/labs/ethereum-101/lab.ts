      ...
      cards: [
        {
          ...
          body: "Most apps you use run on servers owned by a company.\n**Ethereum is different**, it runs on thousands of independent computers around the world, called nodes.\n\nEvery node keeps its own copy of the same shared record: who owns what, and the programs running on the network. When something changes, each node runs the same check on its own, and they all reach the same answer.\n\nEthereum doesn't belong to any company, so there isn't an admin who can simply block a valid transaction, delete an app, or rewrite history. That shared network is the **world computer**.",
        },
        {
          type: "concept",
          ...
          body: "You interact with Ethereum through an **account**, it has two main pieces:\n\n- A public identifier, called an **address**, where others can send assets like ETH (Ethereum's native currency) or tokens.\n- A **private key** that lets you prove you are allowed to use the account. Never share this key. Anyone holding it has total control over your assets.\n\nTo change something on Ethereum, you use a software app called a **wallet** to sign a **transaction** with your key, proving you're the owner, and send it to the network. The network's nodes check the transaction. If it follows the rules, the change is recorded permanently on Ethereum. At that exact moment, the network deducts a small fee called **gas**, which pays for the work the network does and helps stop people from flooding Ethereum with spam.",
        },
        {
          type: "question",
          id: "one-company-ethereum",
          label: "QUESTION",
          title: "What if one company owned Ethereum?",
          question:
            "You just saw that no single company owns Ethereum, but flip it around for a moment: imagine one company owns and runs the whole network. What could they do to you, and to the apps you rely on, that nobody can do today?",
          rubricConcepts: [
        ...
        },
        {
          ...
          body: "A **smart contract** is a program stored on Ethereum. Its rules can usually be inspected, and when someone interacts with it, it follows the rules in its code.\n\nThe classic mental model is a vending machine. You put in the right coin, press a button, and the machine follows its rules: if the payment is valid, it releases the item.",
        },
      ],
    },
    {
