  ...
  chapters: [
    {
      ...
      cards: [
        ...
        },
        {
          ...
          body: "You interact with Ethereum through an **account**, it has two main pieces:\n\n- A public identifier, called an **address**, where others can send **ETH** (native currency) and other assets.\n- A **private key** that lets you prove you are allowed to use the account. Never share this key. Anyone holding it has total control over your assets.\n\nTo change something on Ethereum, you use a local app called a **wallet** to sign a **transaction** with your key. Because the wallet runs locally on your device, your private key never leaves your computer. You then send that signed transaction to the network, the nodes check it, and if it follows the rules, the change is recorded permanently. Transactions are bundled into **blocks**, added one after another in a chain, and that chain is the **blockchain**.",
        },
        {
          type: "concept",
          id: "who-keeps-it-running",
          label: "CONCEPT",
          title: "Who keeps it running?",
          body: "Running thousands of nodes around the world is not free, it takes real machines, electricity, and bandwidth. Some users run standard nodes voluntarily for private, direct access to the blockchain, while the network financially rewards operators who stake 32 ETH to validate transactions.\n\nEvery action on Ethereum costs **gas**, a measure of how much work you are asking the network to do. You pay that gas in ETH. A portion is burned forever, and the remainder goes to the validator who processes your transaction. A simple transfer is cheap, a heavier program costs more, and you pay for the work even when the transaction ends up failing.\n\nThat price does a second job. Because every action costs something, no one can flood the network with junk for free. A computer no one owns, kept running by the people who use it.",
        },
        {
          type: "question",
